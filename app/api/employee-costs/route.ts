...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const result = await query<EmployeeCostRow>(`
      SELECT 
        employee_name,
        employee_id,
        period_start,
    ...
    const summaryResult = await query<EmployeeCostSummaryRow>(`
      SELECT 
    ...
    const projectResult = await query<ProjectCostRow>(`
      SELECT 
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    const { employee_name, period_start, period_end } = await request.json() as { 
    ...
    const detailResult = await query<PayrollDataDetailRow>(`
      SELECT 
        pd.*,
        p.client_name,

...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { filename } = processRequestSchema.parse(body);
    ...
    if (filename) {
      ...
      const result = await processFile(payrollFolderPath, filename);
      results.push(result);
    } else {
      ...
      for (const file of files) {
        ...
        const result = await processFile(payrollFolderPath, file);
        results.push(result);
      }
    }
...
async function processFile(folderPath: string, filename: string): Promise<ProcessingResult> {
  const startTime = Date.now();
  ...
  try {
    ...
    const fileId = (fileRecord[0] as { id: number }).id;
...
async function insertSpringAheadRecord(fileId: number, record: SpringAheadRecord): Promise<void> {
...
async function insertPaychexRecord(fileId: number, record: PaychexRecord): Promise<void> {
  // Calculate burden
...
async function calculateEmployeeCosts(): Promise<void> {
  ...
  await sql`
    ...
    SELECT 
      employee_name,
      DATE_TRUNC('month', COALESCE(work_date, pay_period_start))::date as period_start,
      (DATE_TRUNC('month', COALESCE(work_date, pay_period_start)) + INTERVAL '1 month - 1 day')::date as period_end,
      ...
      '{}'::jsonb as project_allocations
    FROM payroll_data
    GROUP BY employee_name, DATE_TRUNC('month', COALESCE(work_date, pay_period_start))

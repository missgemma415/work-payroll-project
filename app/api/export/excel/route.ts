...
export async function GET() {
  try {
    ...
    const employeeCosts = await query(`
      SELECT 
        employee_name,
        employee_id,
        period_start,
    ...
    const employeeData = employeeCosts.map((row: any) => ({
      ...
      'Burden Rate %': row.burden_percentage ? parseFloat(row.burden_percentage).toFixed(2) : '0.00'
    }));
    ...
    const projectCosts = await query(`
      SELECT 
        ...
        AVG(pd.burden_rate) * 100 as avg_burden_rate
      FROM projects p
    ...
    const projectData = projectCosts.map((row: any) => ({
      ...
      'Avg Burden Rate %': row.avg_burden_rate ? parseFloat(row.avg_burden_rate).toFixed(2) : '0.00'
    }));
    ...
    const detailData = await query(`
      SELECT 
        ...
        pd.total_burden,
        pd.true_cost,
        pd.burden_rate * 100 as burden_percentage,
        pd.source_type,
    ...
    const detailRows = detailData.map((row: any) => ({
      'Employee Name': row.employee_name,
      'Project ID': row.project_identifier || '',
    ...
    const summaryStats = await query(`
      SELECT 
        ...
        SUM(ec.total_true_cost) as total_monthly_cost,
        AVG(ec.total_true_cost) as avg_employee_cost,
        AVG(ec.burden_rate) * 100 as avg_burden_rate,
        SUM(ec.total_hours) as total_hours,
        COUNT(DISTINCT p.project_identifier) as total_projects
      FROM employee_costs ec
      CROSS JOIN (SELECT COUNT(DISTINCT project_identifier) as project_count FROM projects) p
    `);
